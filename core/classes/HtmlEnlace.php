<?php
	class HtmlEnlace{
		...
		public function cargarEnlace($en,$tit,$id){ // Agregamos los diferentes enlaces y los guardamos en los arrays 
			$this->enlaces[]=$en;
			$this->titulos[]=$tit;
			$this->id=$id;
		}
		...
		public function mostrarHorizontal(){ // Mostramos los enlaces horizontalmente
			...
			for($f=0;$f<count($this->enlaces);$f++){ // Recorremos los enlaces para agregarle sus diferentes titulos y enlaces 'href'
				echo '<li style="float:left;"><a href="'.$this->enlaces[$f].'">'.$this->titulos[$f].'</a></li>'; //	float:left tiene una influencia importante para que se muestre de esta manera
			}
			echo '</ul>';
			echo '</div>';
		}
