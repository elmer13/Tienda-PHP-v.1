<?php	
	class HtmlForm{
		...
		public function addInput($type,$name,$value,$atributs = array()){ // Creación de los diferentes inputs teniendo en cuenta el tipo y sus caracteristicas
			...
			echo "<input type=\"".@$this->type."\" name=\"".@$this->name."\" value=\"".@$this->value."\" placeholder=\"".@$this->_atributs["placeholder"]."\" id=\"".@$this->_atributs["id"]."\" class=\"".@$this->_atributs["class"]."\" size=\"".@$this->_atributs["size"]."\"".@$this->_atributs["required"]."/>";
		}
		...
		public function addcheckradio($type,$name,$values,$selected=0){ // Creación de un checkradio pasandole como valor un array y posteriormente guardamos las variables
			$this->value = $values;
			...
			while(list($val,$l)=each($this->value)){ // Asignamos a la variable '$value' los diferentes valores del array      
				...
				echo "<label>".$this->value[$val]."</label><input type='".$this->type."' name='".$this->name."' value='".$val."'".$this->check."<br>";
				$c++;
			}
		}
