<?php
	class Tag{
		...
		public function openTag($type,$id,$class,$attributes = array()){ // Este metodo define la obertura de los diferentes elementos html que podemos crear ej:label, divs, etc
			$this->type = $type;
			$this->id = $id;
			$this->class = $class;
			...
			if(empty($this->class) || $this->class == ""){ $this->class = NULL;}else{ $this->class = "class=\"".@$this->class."\"";}
			if(!empty($this->attributes)){ // Si contiene atributos los implementamos y los mostramos de una manera determinada
				...
				$this->atrr = array();
				echo "<".$this->type." ".@$this->class." ".@$this->id." ".$acciones.">";
			}else{ // Si no contiene atributos mostramos la etiqueta sin esa opción
				echo "<".$this->type." ".@$this->class." ".@$this->id.">";	
			}
		}
